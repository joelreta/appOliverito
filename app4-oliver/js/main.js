...
function queryDetalleSuccess(tx, results) {
	...
	var result = str.link($.registro.pweb);
		...
		$("#DEtelefono").attr("href", "tel:" + $.registro.telefono);
		$("#DEnum_tel").html("Telefono: " +result2);
		$("#DElabel_mail").html("Mail: " + $.registro.email);
		$("#DEnota").html("Nota: " +$.registro.nota);
		$("#DEpweb").html("Pagina Web: " + result);
        $("#id_re").attr("value", $.id);
...
function deleteData(tx) {
    ...
    tx.executeSql("DELETE FROM agenda_curso WHERE id = " + $.id, [], updateFormSuccess, errorDB);
    }
}